...
def createChannel(client):
    client.end_channel(1277364044)
    topic = input("[.] Enter your topic:")
    print("[.] Loading")
    room = client.create_channel(topic)
    print("[!] your room id is: " + room['channel'] + " You can join using it.")
    chat_main(client)

def viewFollow(client,state,max_limit=20):
    console = Console()
    table = Table(show_header=True, header_style="bold magenta")
    table.add_column("user_id", style="cyan", justify="right")
    table.add_column("username")
    ...
    if(users['users']):
        ...
        table = "You dont have any follow/ing List Yet"
    console.print(table)

    return chat_main(client)

def searchUser(client):
    username = input("Enter username:")
    user_id = client.search_users(username)
    if(user_id['users']):
        user_id = user_id['users'][0]['user_id']
        client.follow(user_id)
        print("Followed")
    else:
        print("No username found")
    return flowlist(client)


def flowlist(client):
    print(
          "[.] View Channels PRESS 1 \n"
          "[.] FOR CREATING A CHANNEL PRESS 2 \n"
          "[.] FOR Following list PRESS 3 \n"
          "[.] FOR followers LIST press 4 \n"
          "[.] Search USER and follow press 5 \n"
          )
    user_input = int(input("Enter: "))
    # functionList = {
    #     chat_main(client),
    #     createChannel(client),
    #     viewFollow(client,False),
    #     viewFollow(client,True),
    #     searchUser(client)
    # }
    # functionList[user_input-1]
    # print(functionList)
    if(user_input == 2):
        createChannel(client)
    elif(user_input == 3):
